...
impl Sysproxy {
    ...
    pub fn get_system_proxy() -> Result<Sysproxy> {
        ...
        socks.enable = enable;
        socks.bypass = Sysproxy::get_bypass().unwrap_or_default();
...
fn set_proxy(proxy: &Sysproxy, service: &str) -> Result<()> {
    match env::var("XDG_CURRENT_DESKTOP").unwrap_or_default().as_str() {
        "KDE" => {
            ...
            let schema = format!("{scheme}://{} {}", proxy.host, proxy.port);
        ...
        _ => {
            ...
            gsettings()
                .args(["set", schema.as_str(), "host", host.as_str()])
                .status()?;
            gsettings()
